...
class NoisyEdges {
    ...
    static public function buildNoisyLineSegments(random:PM_PRNG, A:Point, B:Point, C:Point, D:Point, minLength:Float):Array<Point>{
        ...
        function subdivide(A:Point, B:Point, C:Point, D:Point, proceed:Bool = true):Void {
        	if(!proceed)
        	{
        		return;
        	}
            ...
            var s:Float=1.0 - random.nextDoubleRange(-0.4, 0.4);// vertical(along A-D and B-C)
        	var t:Float=1.0 - random.nextDoubleRange(-0.4, 0.4);// horizontal(along A-B and D-C)

            //if(limit-- > 0) {Lib.trace([p, q, s, t]);}


            subdivide(A, Point.interpolate(G, B, s), H, Point.interpolate(E, D, t), false);
            points.push(H);
            subdivide(H, Point.interpolate(F, C, s), C, Point.interpolate(I, D, t), false);
        }

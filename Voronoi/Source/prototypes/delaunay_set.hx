...
package;
  ...
  class delaunay_set extends Sprite {
...
public function go():Void {
  ...
  var triangles:Array<Dynamic>=Delaunay.triangulate(pxyz);

  drawOld(pxyz, triangles);

	  /*
  for(var tri:ITriangle in triangles){
...
public function drawOld(pxyz:Array, triangles:Array):Void {
  for(var tri:ITriangle in triangles){
	  function D(p1:Int, p2:Int, p3:Int):Void {
		var i1:Bool=inside(pxyz[p1]);
		var i2:Bool=inside(pxyz[p2]);
		var i3:Bool=inside(pxyz[p3]);
		if(!i1 && !i2 && i3 && p1>p2){
		  var p:Int=p1;
		  p1=p2;
		  p2=p;
...
public function drawNoisyLine(p:XYZ, q:XYZ, style:Dynamic=null):Void {
...
public function inside(xyz:XYZ):Bool {
  var angle:Float=Math.atan2(xyz.y-250, xyz.x-300);
  var length:Float=new Point(xyz.x-300, xyz.y-250).length;
  var r1:Float=150 - 30*Math.sin(5*angle);
  var r2:Float=150 - 70*Math.sin(5*angle + 0.5);

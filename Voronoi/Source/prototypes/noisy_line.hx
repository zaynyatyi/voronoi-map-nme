...
package;
  ...
  class noisy_line extends Sprite {
...
public static function drawLineP(g:Graphics, A:Point, B:Point, C:Point, D:Point, style:Dynamic):Float {
  return drawLine(g, new Vector3D(A.x, A.y), new Vector3D(B.x, B.y), new Vector3D(C.x, C.y), new Vector3D(D.x, D.y), style);
}


public static function drawLine(g:Graphics, A:Vector3D, B:Vector3D, C:Vector3D, D:Vector3D, style:Dynamic=null):Float {
  if(!style)style={};
  ...
  var minLength:Float=style.minLength !=null? style.minLength:3;
  ...
  var p:Float=random(0.1, 0.9);// vertical(along A-D and B-C)
  var q:Float=random(0.3, 0.7);// horizontal(along A-B and D-C)
  ...
  var s:Float=random(-0.4, 0.4);
  var t:Float=random(-0.4, 0.4);
...
public static function buildLineSegments(A:Point, B:Point, C:Point, D:Point, minLength:Float):Array<Point>{
...
public static function Interpolate(p:Vector3D, q:Vector3D, f:Float):Vector3D {
  return new Vector3D(p.x*(1-f)+ q.x*f, p.y*(1-f)+ q.y*f, p.z*(1-f)+ q.z*f);
}

public static function InterpolateP(p:Point, q:Point, f:Float):Point {

...
package;
  ...
  class quadrilateral_drainage_basin extends Sprite {
...
public function go():Void {
  ...
  if(false){
	...
	var P:Array<Dynamic>=[[100, 20], [200, 200], [160, 300], [200, 500],
	...
	function draw(A:Array, B:Array, C:Array, D:Array):Void {
	  ...
	  normal.normalize();
	  var volume:Float=0.0003 * areaOfQuad(aV, bV, cV, dV);
...
public function areaOfQuad(A:Vector3D, B:Vector3D, C:Vector3D, D:Vector3D):Float {
...
public function drawQuad(A:Vector3D, B:Vector3D, C:Vector3D, D:Vector3D, volume:Float):Void {
  // Draw the quadrilateral
  var lightingVector:Vector3D=D.subtract(C);
  lightingVector.normalize();
  var lighting:Float=1.0 + lightingVector.dotProduct(new Vector3D(-0.3, -0.3, -0.6));
  lighting *=0.6;
  ...
  var p:Float=random(0.5, 0.7);// vertical(along A-D and B-C)
  var q:Float=random(0.1, 0.9);// horizontal(along A-B and D-C)
  ...
  var v0:Float=volume * p;// random(0.5, 1.0);// how much comes from tributaries
  var volumeFromLeft:Float=q * random(1, 2)/ random(1, 2);
  var v1:Float=v0 * volumeFromLeft;
  var v2:Float=v0 - v1;
...
public function drawRiver(g:Graphics, p:Vector3D, q:Vector3D, u:Float, v:Float, color:Int, first:Bool=true):Float {
  if(u<0.25 || v<0.25){
...
public static function random(low:Float, high:Float):Float {
  return low +(high-low)* Math.random();
}


// Interpolate between two points
public function Interpolate(p:Vector3D, q:Vector3D, f:Float):Vector3D {

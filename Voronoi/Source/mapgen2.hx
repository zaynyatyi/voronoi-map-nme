...
private var rotationAnimation:Float=0.0;
...
public function newIsland(type:String):Void {
  ...
  var match:Dynamic=/\s*(\d+)(?:\-(\d+))\s*$/.exec(islandSeedInput.text);
  if(match !=null){
...
public function drawHistograms():Void {
  ...
  function drawHistogram(x:Float, y:Float, bucketFn:Function, colorFn:Function,
  ...
  function drawDistribution(x:Float, y:Float, bucketFn:Function, colorFn:Function,
							width:Float, height:Float):Void {
	var scale:Float, i:Int, x:Float, w:Float;
	var histogram:Array<Dynamic>=computeHistogram(bucketFn);
...
private function InterpolateColor(color0:Int, color1:Int, f:Float):Int {
  var r:Int=uint((1-f)*(color0>>16)+ f*(color1>>16));
  var g:Int=uint((1-f)*((color0>>8)& 0xff)+ f*((color1>>8)& 0xff));
  var b:Int=uint((1-f)*(color0 & 0xff)+ f*(color1 & 0xff));
  if(r>255)r=255;
  if(g>255)g=255;
  if(b>255)b=255;
  return(r<<16)|(g<<8)| b;
...
private function drawGradientTriangle(graphics:Graphics,
  ...
  if(G.length<1e-6){
	...
	m.translate(C.x, C.y);
	var alphas:Array<Dynamic>=colors.map(function(_:Dynamic, index:Int, A:Array):Float { return 1.0;});
	var spread:Array<Dynamic>=colors.map(function(_:Dynamic, index:Int, A:Array):Int { return 255*index/(A.length-1);});
...
// before rendering.
public function render3dPolygons(graphics:Graphics, colors:Dynamic, colorFunction:Function):Void {
  ...
  for(var tri:Dynamic in triangles3d){
...
public function renderPolygons(graphics:Graphics, colors:Dynamic, gradientFillProperty:String, colorOverrideFunction:Function):Void {
  ...
  for(p in map.centers){
	  for(r in p.neighbors){
		  ...
		  if(gradientFillProperty !=null){
			...
			var midpointAttr:Float=0.5*(corner0[gradientFillProperty]+corner1[gradientFillProperty]);
...
public function makeExport(layer:String):ByteArray {
  ...
  var exportData:ByteArray<Dynamic>=new ByteArray();
  ...
  if(layer=='overrides'){
	...
	for(var p:Center in map.centers){
		if(!p.ocean){
